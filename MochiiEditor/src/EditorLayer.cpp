...
void EditorLayer::OnAttach() {
  ...
  fbSpec.Width = 1280;
  fbSpec.Height = 720;
  m_Framebuffer = Mochii::Framebuffer::Create(fbSpec);
}

void EditorLayer::OnDetach() { MI_PROFILE_FUNCTION(); }

void EditorLayer::OnUpdate(Mochii::Timestep ts) {
  MI_PROFILE_FUNCTION();

  // Resize
  if (Mochii::FramebufferSpecification spec = m_Framebuffer->GetSpecification();
...
void EditorLayer::OnImGuiRender() {
  ...
  if (ImGui::BeginMenuBar()) {
    if (ImGui::BeginMenu("File")) {
      ...
      if (ImGui::MenuItem("Exit")) Mochii::Application::Get().Close();
      ImGui::EndMenu();
    }
...
void EditorLayer::OnEvent(Mochii::Event& e) { m_CameraController.OnEvent(e); }
}  // namespace Mochii
